...
CODE=""; SECRET=""; STATION=""; BROKER=""; DL_IP=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --code) CODE="$2"; shift 2;;
...
EXPERANTO_EDGE_CONFIG="$CFG" "$APP_DIR/venv/bin/experanto-edge" --enroll "$ENROLL" || true
...
# The unit runs the console script; point EXPERANTO_EDGE_CONFIG at our config file.
...
ExecStart=
ExecStart=$APP_DIR/venv/bin/experanto-edge
EOF
